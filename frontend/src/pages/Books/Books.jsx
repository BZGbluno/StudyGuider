      ...
      <section className="w-full max-w-2xl">
        <header className="text-center mb-6">
          <h1 className="text-4xl font-extrabold">
            Welcome, {user?.user_metadata?.firstName || user?.email || "User"}!
          </h1>
        ...
        <div className="bg-[#0b0b0b] border border-gray-800 rounded-2xl p-6 space-y-4 shadow-xl">
          ...
          <div>
            <button
              ...
              className="w-full p-8 min-h-[80px] rounded-xl border-2 border-dashed border-gray-700 text-center text-white bg-transparent hover:bg-white/3 transition disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-transparent"
            >
              <p className="font-semibold text-center flex items-center justify-center">
                Import New Textbook{" "}
                <span className="text-4xl ml-4 mb-1">+</span>
              </p>
            </button>
            {books.length >= MAX_TEXTBOOKS && (
              <p className="mt-3 text-sm text-gray-400 text-center">
                You can only have {MAX_TEXTBOOKS} textbooks. Delete one to
                upload another.
              </p>
            )}
          </div>
